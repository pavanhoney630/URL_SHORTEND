...
const LinkPage = ({ searchQuery }) => {
  ...
  const handleUpdate = async () => {
    try {
      const response = await axios.put(
        ...
        { newOriginalUrl }, // Send only the updated original URL
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
        ...
        prevLinks.map((link) =>
          link.shortenedUrl === editUrl
            ? {
                ...
                shortenedUrl: updatedLink.shortenedUrl,
              }
            : link
        )
      );
      ...
      setEditUrl(null); // Clear the editUrl after update
      setPopup("edit"); // Close popup
    } catch (err) {
  ...
  const handleRedirect = async (shortenedUrl) => {
    try {
      ...
      if (updatedLink.expirationDate && new Date(updatedLink.expirationDate) < new Date()) {
  ...
  const getStatusColor = (expirationDate) => {
    return expirationDate && new Date(expirationDate) < new Date() ? "red" : "green";
  };
            ...
            {filteredLinks.map((link) => (
              <tr key={link.shortenedUrl}>
                ...
                <td>
                  ...
                  <FaTrash
                    onClick={() =>
                      setPopup({ type: "delete", shortenedUrl: link.shortenedUrl })
                    }
                    className={styles.icon}
                  />
                </td>
      ...
      {popup?.type === "delete" && (
        <div className={styles.popup}>
          <div className={styles.popupContent}>
            ...
            <button onClick={() => handleDelete(popup.shortenedUrl)}>Yes</button>
            <button onClick={() => setPopup(null)}>No</button>
          </div>
        </div>
      )}
    </div>
