...
const SettingsPage = () => {
  ...
  const validateForm = () => {
    ...
    if (!mobile || !/^\d{10}$/.test(mobile)) newErrors.mobile = 'Please enter a valid mobile number (10 digits)';
    setErrors(newErrors);
  ...
  const handleSaveChanges = async (e) => {
    ...
    try {
      ...
      alert(error.response ? error.response.data.message : 'Failed to update account details.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDeleteAccount = async () => {
    ...
    if (window.confirm('Are you sure you want to delete your account? This action is irreversible.')) {
      try {
        ...
        alert(error.response ? error.response.data.message : 'Failed to delete account.');
      } finally {
        setLoading(false);
      }
    }
      ...
      <form onSubmit={handleSaveChanges} className={styles.form}>
        <div className={styles.formGroup}>
          ...
          <input
            ...
            className={`${styles.input} ${errors.name ? styles.errorInput : ''}`}  // Conditionally add error class
          />
        ...
        <div className={styles.formGroup}>
          ...
          <input
            ...
            value={email}
            onChange={handleInputChange}
            className={`${styles.input} ${errors.email ? styles.errorInput : ''}`} // Conditionally add error class
          />
        ...
        <div className={styles.formGroup}>
          ...
          <input
            ...
            value={mobile}
            onChange={handleInputChange}
            className={`${styles.input} ${errors.mobile ? styles.errorInput : ''}`}  // Conditionally add error class
          />
        ...
        <div className={styles.buttons}>
          ...
          <button type="button" onClick={handleDeleteAccount} className={styles.deleteButton} disabled={loading}>
